...
# some of the functions in this script comes from osm.pm module written by Gerhard Schwanz
# Parameters:
# <1> - imported file
...
sub getFmRelHikingTags {
	...
	my $fmRelTags = {};
	my $symbol = getSymbol($relTags);
	my $osmcsymbol = getOSMCSymbol($relTags);
...
##	if (!isValidHikingSymbol($symbol)) { return $fmRelTags; }	# avoid strange relations
    ...
    $fmRelTags->{"fmreleducationname"} = $relTags->{"name"};
		...
		if (defined($relTags->{"operator"})) { $operator = $relTags->{"operator"}; }
		if ($network eq "lwn") {
			$network = "local";
...
sub getOSMCSymbol {
	...
	if (defined($relTags->{"osmc:symbol"})) { $osmcsymbol = $relTags->{"osmc:symbol"}; }
	if ($osmcsymbol eq "") { $osmcsymbol = "none"; }
	$osmcsymbol =~ s/[a-z]*:[a-z]*:[a-z]*_//;
...
sub isValidHikingSymbol {
	...
	return grep(/\Q^$symbol$/, @allowedHikingSymbols);	
}
